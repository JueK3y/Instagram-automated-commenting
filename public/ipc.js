...
ipc.on("getCurVer", (event, text) => {
  ...
  if (text.includes("alpha"))
    document.getElementById("verType").innerText = "Alpha release";
  else if (text.includes("beta"))
    document.getElementById("verType").innerText = "Beta release";
...
ipc.on("blurPw", () => {
  ...
  document.getElementById("togglePwImage").src =
    "src/img/icons/" + document.body.classList + "/eye.svg";
});
  ...
  if (validate) {
    if (checkClick === 1) {
      ...
      showBanner(
        "warning",
        "IAC kommentiert",
        "Sicher, dass du das Kommentieren abbrechen willst?",
        "close-while-commenting",
        true
      );
      checkClick++;
    }
