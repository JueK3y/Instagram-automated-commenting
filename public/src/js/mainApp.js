...
function launchMainLogic(_url, _username, _password, _mode) {
  puppeteer
    ...
    .then(async (browser) => {
      // TODO: Without slowMo arg in production -!- //
      while (runMainLogic) {
      ...
      await page.screenshot({ path: 'Stealth.png' })*/
        ...
        if (postURL.slice(0, 4) !== "http") {
          // INFO: Doesn't check, if :// is already there -!- //
        ...
        if (runMainLogic) {
          // TODO: Better stillRunningCheck needed -!- //
          if ((await page.$(".HoLwm")) !== null) {
            // INFO: Shouldn't use class detection -!- //
        ...
        if (runMainLogic) await page.click('[type="submit"]');
        // TODO: Better stillRunningCheck needed -!- //
        else {
          await page.close();
          return;
        ...
        if (runMainLogic) await page.waitForTimeout(5000);
        // TODO: Better stillRunningCheck needed -!- //
        else {
          await page.close();
          return;
        ...
        if ((await page.url()) === loginURL) {
          ...
          log.warn(
            'Instagram error message: "' +
              ...
              '"'
          ); // FIXME: Eval is considered as unsafe -!- //
          noteMessage(
            "Falsche LogIn Daten",
            "Bitte überprüfe die eingegebenen LogIn Daten und probiere es erneut.",
            true
          );
          showBanner(
            "error",
            "Falsche Eingabe?",
            "Bitte überprüfe die angegebenen LogIn Daten.",
            "wrong-login-data",
            true
          );
          ...
          if ((await page.url()) === mfaURL) {
            ...
            noteMessage(
              "Zwei-Faktor Authentifizierung entdeckt",
              "In dieser Version wird das Anmelden mit 2FA noch nicht unterstützt",
              true
            );
            showBanner(
              "warning",
              "2FA LogIn",
              "2FA Login zur Zeit nicht möglich.",
              "multi-factor-auth",
              true
            );
            document.getElementById("stop-btn").click();
          ...
          showBanner(
            "info",
            "LogIn erfolgreich",
            "Das Einloggen in Instagram war erfolgreich.",
            "login-success",
            true
          );
          ...
          try {
            ...
            showBanner(
              "error",
              "Ladefehler",
              "Die URL konnte nicht geladen werden. Bitte erneut versuchen.",
              "post-timeout",
              true
            );
            document.getElementById("stop-btn").click();
          ...
          if (runMainLogic)
            getComments(); // TODO: Better stillRunningCheck needed -!- //
          else {
            await page.close();
            return;
          }
                  ...
                  try {
                    if (spamNotice !== null) {
                      showBanner(
                        "warning",
                        "Spam erkannt",
                        "IAC 2.0 muss etwas langsamer kommentieren.",
                        "spam-notice",
                        true
                      );
                    ...
                    noteMessage(
                      "Falsche URL?",
                      "Bitte überprüfe die URL und probiere es erneut.",
                      true
                    );
                    showBanner(
                      "error",
                      "Falsche URL?",
                      "Bitte URL überprüfen und erneut versuchen.",
                      "wrong-ig-url",
                      true
                    );
                    document.getElementById("stop-btn").click();
              ...
              try {
                if (runMainLogic) {
                  ...
                  if (i === comment.length - 1) {
                    ...
                    noteMessage(
                      "Kommentieren abgeschlossen",
                      "IAC 2.0 hat alle Kommentare erfolgreich gepostet.",
                      true
                    );
                    showBanner(
                      "info",
                      "Kommentieren fertig",
                      "Das Kommentieren wurde erfolgreich abgeschlossen.",
