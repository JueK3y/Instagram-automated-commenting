...
document.getElementById("closeBtn").addEventListener("mouseover", function () {
  document.getElementById("closeBtnIcon").src =
    "src/img/icons/dark/win-close.svg";
});
document.getElementById("closeBtn").addEventListener("mouseout", function () {
  if (body.classList.contains("light")) {
    document.getElementById("closeBtnIcon").src =
      "src/img/icons/light/win-close.svg";
  }
});
...
function lineAnimation2(type, marPx, heiPx) {
  $(".line").animate(
    { marginBottom: "-40px", height: "40px", opacity: "0" },
    250
  );
  setTimeout(() => {
    $("#active").removeAttr("id");
...
$(document).ready(() => {
  // TODO: Must be modified so that button click also has a timeout -!- //
  $("body").click(function (e) {
    if ($("#active").hasClass("profile")) {
      if (
        !$(e.target).closest("#profile-container").length &&
    ...
    if ($("#active").hasClass("helpNav")) {
      ...
      if (
        !$(e.target).closest("#help-container").length &&
        !$(e.target).closest("#nav-bar").length
      ) {
        $(".comment").click();
    ...
    if ($("#active").hasClass("settings")) {
      if (
        !$(e.target).closest("#settings-container").length &&
        !$(e.target).closest("#nav-bar").length
      )
        $(".comment").click();
    }
...
$(document).ready(() => {
  ...
  $(".comment").click(() => {
    if (!$("#active").hasClass("comment") && $("#active").hasClass("profile")) {
      ...
      !$("#active").hasClass("comment") &&
      ($("#active").hasClass("helpNav") || $("#active").hasClass("settings"))
    ) {
      if (winOpen) closeSmallWin();
      if (locked) return;
      locked = true;
  ...
  $(".profile").click(() => {
    if (!$("#active").hasClass("profile") && $("#active").hasClass("comment")) {
      ...
      (!$("#active").hasClass("profile") && $("#active").hasClass("helpNav")) ||
      $("#active").hasClass("settings")
    ) {
      if (winOpen) closeSmallWin();
      if (locked) return;
      locked = true;
  ...
  $(".helpNav").click(() => {
    if (
      (!$("#active").hasClass("helpNav") && $("#active").hasClass("comment")) ||
      $("#active").hasClass("profile")
    ) {
      if (locked) return;
      locked = true;
      ...
      !$("#active").hasClass("helpNav") &&
      $("#active").hasClass("settings")
    ) {
      if (locked) return;
      locked = true;
      $(".line").animate(
        { marginTop: "-40px", height: "40px", opacity: "0" },
        300
      );
      setTimeout(() => {
        $("#active").removeAttr("id");
  ...
  $(".settings").click(() => {
    if (
      ...
      locked = true;
      $(".line").animate(
        { height: "40px", marginBottom: "-40px", opacity: "0" },
        250
      );
      setTimeout(() => {
        $("#active").removeAttr("id");
        ...
        $(".line").css("margin-top", "-38px");
        $(".line").animate(
          { marginTop: "5px", height: "19px", opacity: "1" },
          300
        );
      }, 270);
      setTimeout(() => {
...
for (let i = 1; i <= 3; i++) {
  document.getElementById("delete-" + i).onmouseover = () => {
    document.getElementById("deleteIcon-" + i).src =
      "src/img/icons/" + document.body.classList + "/delete-red.svg";
  };
  document.getElementById("delete-" + i).onmouseleave = () => {
    document.getElementById("deleteIcon-" + i).src =
      "src/img/icons/" + document.body.classList + "/delete.svg";
  };
}
  ...
  getNetworkDownloadSpeed().then((result) => {
    ...
    if (notConnected) {
      ...
      if (internetSpeed === 0) {
        ...
        changeColor.style.background = "none";
        changeImg.src =
          "src/img/icons/" + document.body.classList + "/wifi/wifi-good.svg";
      }
    }
      ...
      if (counterDisplay === 3 || notConnected) {
        ...
        if (updateOnline) {
          ...
          showBanner(
            "info",
            "Update gefunden",
            "Es gibt eine neue Version für IAC.",
            "update-found-search",
            true
          );
          noteMessage(
            "Update für IAC",
            "Es wurde eine neue Version für IAC 2.0 gefunden. Jetzt installieren?"
          );
        } else {
          ...
          showBanner(
            "info",
            "Aktuelleste Version",
            "Kein Update gefunden. IAC ist auf dem neusten Stand.",
            "no-update-found",
            true
          );
          setTimeout(() => {
    ...
    if (counterDisplay === 3 || notConnected) {
      ...
      if (downloadSuccess) {
        showBanner(
          ...
          "IAC wird jetzt geupdatet und wird ggfl. neugestartet.",
          "install-update",
          true
        );
        download.style.display = "none ";
        ...
        showBanner(
          "error",
          "Fehlgeschlagen",
          "Die neuste Version konnte nicht heruntergeladen werden.",
          "install-update-failed",
          true
        );
        noteMessage(
          "Herunterladen fehlgeschlagen",
          "Die neuste Version konnte nicht heruntergeladen werden. Probiere es manuell.",
          true
        );
        download.style.display = "none";
        downloadFailed.style.display = "inline-block";
...
$(document).on("click", "#report-bug", () => {
  openSmallWin(
    "template",
    "src/img/icons/dark/bug.svg",
    "Fehler melden",
    "Falls dir ein Fehler, wie zum Beispiel ein Übersetzungs-, Anzeige oder Logikfehler aufgefallen ist, kannst du diesen hier melden. Bei Klick auf Melden wirst du auf GitHub weitergeleitet.",
    "Melden",
    "https://github.com/JueK3y/Instagram-automated-commenting/issues/new/choose"
  );
});
...
$(document).on("click", "#s-w-c-b-multi-fa", () => {
  ...
  showBanner(
    "error",
    "Fehlender 2FA Code",
    "IAC 2.0 kann ohne den 2FA Code nicht kommentieren.",
    "no-mfa-code",
    true
  );
...
$(document).on("click", "#s-w-m-b-multi-fa", () => {
  ...
  if (mfaCode.length >= 6) {
    ...
    showBanner(
      "warning",
      "Fehlender 2FA Code",
      "Es wird der 6-stellige 2FA Code benötigt.",
      "empty-mfa-input",
      true
    );
...
function closeSmallWin() {
  if (
    document.getElementById("small-window-multi-fa").style.display !== "block"
  ) {
...
function openSmallWin(type, icon, title, content, mainBtn, hyperlink) {
  log.info(
    `Opening SmallWindow with following values: ${type}, ${icon}, ${title}, ${content}, ${mainBtn}, ${hyperlink} `
  );
