const IDs = [
  ...
  "profileDropdownImageNoFocus",
  "deleteIcon-1",
  "deleteIcon-2",
  "deleteIcon-3",
  "small-window-icon-img",
];
...
function lightIcon() {
  ...
  document
    .querySelector("body")
    .style.setProperty("--accent-default-rgb", baseRGB);
  document
    .querySelector("body")
    .style.setProperty(
      "--accent-light-rgb",
      baseRGB[0] + "," + (baseRGB[1] + 20) + "," + baseRGB[2]
    );
  document
    .querySelector("body")
    .style.setProperty(
      "--accent-dark-rgb",
      baseRGB[0] + "," + (baseRGB[1] - 35) + "," + (baseRGB[2] - 35)
    );

  for (let i = 0; i < IDs.length; i++) {
    document.getElementById(IDs[i]).src = document
  ...
  try {
    if (document.getElementById("toggle").checked) {
      ...
      togglePw.src = "src/img/icons/light/eye.svg";
    }
  ...
  return (
    ...
    colorHex
      ...
      .replace(/../g, (colorHex) =>
        (
          "0" +
          Math.min(255, Math.max(0, parseInt(colorHex, 16) + amount)).toString(
            16
          )
        ).substr(-2)
      )
...
function darkIcon() {
  ...
  document
    .querySelector("body")
    .style.setProperty("--accent-default-rgb", darkRGB);
  document
    .querySelector("body")
    .style.setProperty(
      "--accent-light-rgb",
      darkRGB[0] + "," + (darkRGB[1] + 20) + "," + darkRGB[2]
    );
  document
    .querySelector("body")
    .style.setProperty(
      "--accent-dark-rgb",
      darkRGB[0] + "," + (darkRGB[1] - 35) + "," + (darkRGB[2] - 35)
    );
  ...
  try {
    if (document.getElementById("toggle").checked) {
      ...
      togglePw.src = "src/img/icons/dark/eye.svg";
    }
